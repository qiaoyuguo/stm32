...
{
	/* Set GPIO_PIN_LED (in GPIO port C) to 'output push-pull'. */
	gpio_set_mode(GPIO_PORT_LED, GPIO_MODE_OUTPUT_2_MHZ,
...
int main(void)
{
	...
	/* Blink the LED (PF6) on the board with every transmitted byte. */
	while (1) {
		gpio_toggle(GPIO_PORT_LED, GPIO_PIN_LED);	/* LED on/off */
